	...
	$.ajax({
		...
		.done(function(weatherData) {
			...
			for (var i = 1; i < 35; i += 8) {
				...
				//append wanted data from api to html
				$('#slot-' + i).append('<h5>' + littleWeatherDate + '</h5>');
				$('#slot-' + i).append('<img src=' + weatherIcon + '>');
				$('#slot-' + i).append('<h5>' + 'Temperature: ' + weatherData.list[i].main.temp + '&deg;F' + '</h5>');
				$('#slot-' + i).append('<h5>' + 'Humidity: ' + weatherData.list[i].main.humidity + '%' + '</h5>');
			}
